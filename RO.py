...
import pandas as pd
import datetime
# from RO import By, NoSuchElementException, Select, gspread_connect_to_sheet, gspread_get_RO_data, last_login_col, logging, pd, stationName_col, userpass_col, utils
from init import bootstrap

from gspread_functions import gspread_connect_to_sheet, gspread_get_RO_data, gspread_getbool, \
                    gspread_set_RO_transfer_details, gspread_update_REGOs, gspread_update_ROCs
...
def cancel_all_transfers():
    ...
    REGO_sup_num, \
    REGOs_sent, \
    REGOs_sent_date = gspread_get_RO_data(
...
def gspread_set_RO_stats():
    ...
    input(f"Is the 'Number of REGOs pending transfer' column still {utils.rowcol_to_a1(1,REGOs_pending_transfer_col)}?")
    input(f"Is the 'Oldest date of REGOs pending transfer' column still {utils.rowcol_to_a1(1, oldest_REGOs_pending_col)}?")
    input(f"Is the 'Oldest ROCs not issued' column still {utils.rowcol_to_a1(1,oldest_ROCs_not_issued_col)}?")
    input(f"Is the 'Oldest REGOs not issued' column still {utils.rowcol_to_a1(1,oldest_REGOs_not_issued_col)}?")
    input(f"Is the 'Number of ROCs pending transfer' column still {utils.rowcol_to_a1(1,ROCs_pending_transfer_col)}?")
    input(f"Is the 'Oldest date of ROCs pending transfer' column still {utils.rowcol_to_a1(1,oldest_ROCs_pending_col)}?")
    input(f"Is the 'Last Login successful' column still {utils.rowcol_to_a1(1,last_login_col)}?")

    return oldest_REGOs_not_issued_col,oldest_ROCs_not_issued_col,ROCs_pending_transfer_col,REGOs_pending_transfer_col,\
        oldest_ROCs_pending_col,oldest_REGOs_pending_col


def gspread_get_RO_stats(gspread_set_stats):
    oldest_REGOs_not_issued_col, oldest_ROCs_not_issued_col, ROCs_pending_transfer_col, REGOs_pending_transfer_col, \
    oldest_ROCs_pending_col, oldest_REGOs_pending_col = gspread_set_stats()
    ...
    return worksheet,stationNames,usernames,passwords,last_login_succesful,oldest_ROCs_not_issued,oldest_REGOs_not_issued,ROCs_pending_transfer,REGOs_pending_transfer,oldest_ROCs_pending,oldest_REGOs_pending
...
def get_pending_certificates(station_name, username, driver):
    ...
    for cert in ['roc', 'rego']:
        driver.get(f"https://renewablesandchp.ofgem.gov.uk/CertificateTransfer/TransferRequestView.aspx?scheme={cert}")

        try:
            pending_transfers_element = driver.find_element(By.ID, "ctl00_ContentPlaceHolder_gvCertificates")

            # Get the HTML content of the table
            pending_transfers_html = pending_transfers_element.get_attribute("outerHTML")

            # Use Pandas to read HTML and convert it to a DataFrame
            pending_transfers = pd.read_html(pending_transfers_html)[0]
            logging.debug(pending_transfers.head())

            def this_station(frame_row): #check pending transfers are for selected station
                return frame_row['Generator'] == station_name

            pending_transfers_this_station = pending_transfers[pending_transfers.apply(this_station, axis=1)]
            if not(pending_transfers_this_station.empty):
                pending_transfers_this_station.loc[:, 'Output Period'] = pd.to_datetime(pending_transfers_this_station['Output Period']
                                                                                    , format='%b %Y')

                if cert == 'roc':
                    ROCs_pending = int(pending_transfers_this_station['Certificates'].sum())
                    ROCs_pending_date = pending_transfers_this_station['Output Period'].min().strftime('%b %Y')
                    logging.info(f"There are {ROCs_pending} pending {cert.upper()} transfers found for {station_name}")
                    logging.info(f"The oldest pending {cert.upper()} transfer for {station_name} is from {ROCs_pending_date}")

                if cert == 'rego':
                    REGOs_pending = int(pending_transfers_this_station['Certificates'].sum())
                    REGOs_pending_date = pending_transfers_this_station['Output Period'].min().strftime('%b %Y')
                    logging.info(f"There are {REGOs_pending} pending {cert.upper()} transfers found for {station_name}")
                    logging.info(f"The oldest pending {cert.upper()} transfer for {station_name} is from {REGOs_pending_date}")
            else:
                raise NoSuchElementException

        except NoSuchElementException: # if there is no table
            logging.info(f"No {cert.upper()} pending transfers found for {station_name}")

    return ROCs_pending, REGOs_pending, ROCs_pending_date, REGOs_pending_date


def get_oldest_not_issued_dates(station_name, username, ROCs_not_issued_date, REGOs_not_issued_date, driver):

    driver.get("https://renewablesandchp.ofgem.gov.uk/OutputData/ViewOutputHistory.aspx")
    ROCs_not_issued_date='NA'; REGOs_not_issued_date='NA'

    #select generating station
    station_selector = driver.find_element(By.ID, "ctl00_ContentPlaceHolder_GeneratorStation1_ddlStation")
    ...
    try:
        ...
        for option in select.options:
            ...
            if (valid_generator == station_name):
                logging.debug(station_name + " selected")
                #select generator
                value = option.get_attribute("value")
                select.select_by_value(value)
                found = True
        ...
        startDateSelector = driver.find_element(By.ID, "ctl00_ContentPlaceHolder_ddlFromDate")
        select = Select(startDateSelector)
        select.select_by_index(1) #select earliest date

        #apply filter
        applyButton = driver.find_element(By.ID, "ctl00_ContentPlaceHolder_btnFilterResults")
        applyButton.click()

        try:
            output_history_element = driver.find_element(By.ID, "ctl00_ContentPlaceHolder_dgOutputHistory")

            # Get the HTML content of the table
            output_history_html = output_history_element.get_attribute("outerHTML")

            # Use Pandas to read HTML and convert it to a DataFrame
            output_history = pd.read_html(output_history_html)[0]
            logging.debug(output_history.head())

            def ROCs_not_issued(frame_row): #set conditions for ROCs not issued
                return not(any(group == 'Issued' for group in re.findall(r'RO\(([^)]+)\)', frame_row['Scheme'])) or frame_row['Total Eligible Output'] == 0)

            def REGOs_not_issued(frame_row): #set conditions for REGOs not issued
                return not(any(group == 'Issued' for group in re.findall(r'REGO\(([^)]+)\)', frame_row['Scheme'])) or frame_row['Total Eligible Output'] == 0)

            try:
                ROCs_not_issued_index = output_history[output_history.apply(ROCs_not_issued, axis=1)].index[-1]
                ROCs_not_issued_date = pd.to_datetime(output_history.at[ROCs_not_issued_index, 'Period start date']).strftime('%b %Y')
                logging.info(f"{station_name} oldest ROCs not issued for Output Period {ROCs_not_issued_date}")
            except IndexError:
                logging.info(f"{station_name} has no ROCs not issued")
                ROCs_not_issued_date = "NA"

            try:
                REGOs_not_issued_index = output_history[output_history.apply(REGOs_not_issued, axis=1)].index[-1]
                REGOs_not_issued_date = pd.to_datetime(output_history.at[REGOs_not_issued_index, 'Period start date']).strftime('%b %Y')
                logging.info(f"{station_name} oldest REGOs not issued for Output Period {REGOs_not_issued_date}")
            except IndexError:
                logging.info(f"{station_name} has no REGOs not issued")
                REGOs_not_issued_date = "NA"

        except NoSuchElementException: # if there is no table
            logging.info(f"No {type.upper()} output history found for {station_name}")

    except NoSuchElementException:
        logging.warning(f"{station_name} not found on account {username}")
...
if __name__ == "__main__":
    ...
    bootstrap(local_logging=True)

    stationName_col, userpass_col, ROC_sup_num_col, ROCs_sent_col, ROCs_sent_date_col,\
    REGO_sup_num_col, REGOs_sent_col, REGOs_sent_date_col, last_login_col = gspread_set_RO_transfer_details()
    ...
    REGO_sup_num, \
    REGOs_sent, \
    REGOs_sent_date, _, _, _, _, _, _ = gspread_get_RO_data(
    stationName_col=stationName_col,
    userpass_col=userpass_col,
    ROC_sup_num_col=ROC_sup_num_col,
    ROCs_sent_col=ROCs_sent_col,
