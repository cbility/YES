...
def gspread_connect_to_sheet(doc_name, sheet_name):
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name(
        r"C:\Users\Energysource2014\Documents\yes-sheets-85a4f1a3bf09.json", scope
    )
...
def gspread_update_ROCs(ROCs_sent_col, ROCs_sent, ROCs_sent_date_col, ROCs_sent_date):
    worksheet, client = gspread_connect_to_sheet(
        "ES Ongoing Support Services", "ROC FiT Mtrs"
    )

    worksheet.update(
        utils.rowcol_to_a1(2, ROCs_sent_col)
        + ":"
    ...
    worksheet.update(
        utils.rowcol_to_a1(2, ROCs_sent_date_col)
        + ":"
...
def gspread_update_REGOs(
    REGOs_sent_col, REGOs_sent, REGOs_sent_date_col, REGOs_sent_date
):
    worksheet, client = gspread_connect_to_sheet(
        "ES Ongoing Support Services", "ROC FiT Mtrs"
    )
    ...
    worksheet.update(
        utils.rowcol_to_a1(2, REGOs_sent_col)
        + ":"
    ...
    worksheet.update(
        utils.rowcol_to_a1(2, REGOs_sent_date_col)
        + ":"
...
def gspread_set_RO_transfer_details():
    stationName_col = utils.a1_to_rowcol("C1")[1]
    userpass_col = 4
...
def gspread_update_stats(
    ...
    (
        ...
        oldest_REGOs_pending_col,
    ) = RO.gspread_set_RO_stats(check=False)

    stats = [
        [oldest_ROCs_not_issued, oldest_ROCs_not_issued_col],
...
def gspread_get_RHI_users(rhi_col, userpass_col, last_login_col, dates_col, fdates_col):
    ...
    rhi_users, last_login_succesful, old_dates, first_dates = gspread_get_RHI_logins(
...
def gspread_get_RHI_logins(
    rhi_column, username_column, last_login_column, dates_column, fdates_col, worksheet
):
    ...
    for [username, indices] in condense_list_with_indices(usernames):
        rhi_users.append(
            [
                ...
                passwords[indices[0]],
                all(
                    [gspread_getbool(last_login_succesful[index]) for index in indices]
                ),
                [rhis[i] for i in indices],
                [old_dates[i] for i in indices],
                indices,
            ]
        )

    return rhi_users  # , first_dates
