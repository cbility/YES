...
class App:
    ...
    def __init__(
        ...
        if get_data:
            ...
            self.solution_id = response[
                "solution"
            ]  # Extract the associated solution ID
            ...
            self.fields = {
                field_label: Field(
                    field_keys.get("slug", None),
                    field_keys.get("field_type", None),
                    field_keys,
                )
                for field_label, field_keys in fields.items()
            }


# Initialize App objects for RHIs and ORGs with their respective IDs
RHIs = App(
    app_id="64d155a9c71c81dc0b41d52d",
    app_name="RHIs",
    fields={
        "RHI Number": {"slug": "title"},
        ...
        "Last Submitted": {"slug": "s7a7a2ede6"},
    },
)

ORGs = App(
    app_id="64d155a9c71c81dc0b41d527",
    app_name="ORGs",
    fields={
        "Record ID": {"slug": "sa91f18b97"},
        "Username": {"slug": "title"},
...
def get_record_details(
    ...
    if sort_fields:
        body["sort"] = [
            {"field": field, "direction": sort_order} for field in sort_fields
        ]
    ...
    records = [
        {field: record[field] for field in return_fields}
...
def get_RHIs_to_update(RHI_fields_to_get: list):
    """
    Fetches and organizes RHI numbers, usernames, and passwords from the SmartSuite system.

    Args:
        RHIs_fields_to_get: list of IDs of RHI fields to populate in returned records.
    ...
    """
    for RHI_field in RHI_fields_to_get:
        if (
            RHI_field == RHIs.fields["RHI Number"].id
            or RHI_field == RHIs.fields["ORG"].id
            or RHI_field == RHIs.fields["Record ID"].id
        ):
            RHI_fields_to_get.remove(RHI_field)

    # Fetch RHI details with specific fields and filter based on 'Update Data' flag
    RHI_details = get_record_details(
        RHIs.app_id,
        [
            RHIs.fields["RHI Number"].id,
            RHIs.fields["ORG"].id,
            RHIs.fields["Record ID"].id,
            *RHI_fields_to_get,
        ],
    ...
    RHIs_grouped_by_ORG = group(
        RHI_details, RHIs.fields["ORG"].id
    )  # Groups RHIs by their corresponding ORG IDs
    ...
    for record in list_to_group:
        ...
        for group_value in record.get(key_to_group_by):
            ...
            if group_value not in grouped_values:
                ...
                grouped_list.append(
                    {key_to_group_by: group_value, "items": records_in_group}
                )
...
def hydrate(
    list_to_hydrate: list,
    hydrate_target_field: str,
    hydrate_from_app_id: str,
    record_ID_field: str,
    field_IDs_to_populate: list,
):
    """
    ...
    record_details = get_record_details(
        ...
        filter_fields={
            record_ID_field: [
                record[hydrate_target_field] for record in list_to_hydrate
            ]
        },
        operator="or",
    )
    ...
    for record in list_to_hydrate:
        hydrated_list.append(
            {
                field: hydrating_dict[value] if field == hydrate_target_field else value
...
get_record_details("64d155a8c71c81dc0b41d361", ["title"], {"sd136afabc": ["2401528"]})
