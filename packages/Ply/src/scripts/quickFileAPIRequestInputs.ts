//used to replace a Ply sub feature that used to provide the same functionality
//Ply sub features are not available on our plan

export default async function quickFileAPIRequestInputs(input: { endpoint: string; body: string; APIKey: any; }) {
    ...
    async function requestInputs(endpoint: string, //exclude "https://api.quickfile.co.uk"
        body: string) {
        const url = "https://api.quickfile.co.uk" + endpoint;
        const SubmissionNumber = uniqueId();
        const accNumber = "6131405563";
        const applicationID = "8e48b467-333a-46cc-958c-a2f286dad614"; //Ply  Application ID
...
//USAGE: return quickFileAPIRequestInputs(input);